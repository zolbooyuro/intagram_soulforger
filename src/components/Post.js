...
const Post = (nani) => {
    ...
    const number = nani.number;
    const name = nani.name;
    const image = nani.image;
    const liked = nani.like
    const [likedNumber, setLikedNumber] = useState(liked.length);
    ...
    useEffect(() => {
        if (like === true) {
            var cnt = 0;
            liked.forEach(element => {
                cnt = element === localStorage.getItem('name') ? cnt + 1 : cnt;
            })
            if (cnt === 0) {
                setLikedNumber(likedNumber + 1)
                var array = liked;
                array.push(localStorage.getItem('name'))
                firestore.collection('posts').doc('' + number).update({
                    liked : array
                })
            }
        } else {
            ...
            if (cnt !== 0) {
                var array = liked;
                const index = array.indexOf(localStorage.getItem('name'));
                if (index > -1) {
                        ...
                        <div className="nav-wrapper">
                            <form>
                                <div className="input-field">
                                    <input placeholder="Add a comment..." id="comment" type="search" required />
